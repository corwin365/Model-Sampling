...
%path handling
Settings.Instrument = 'limb_merged';
[~,CoreSettings] = sampling_core_v3(' ',' ',0,'GetSettings',true);
Settings.OutDir  = [CoreSettings.MasterPath,'/tracks/',Settings.Instrument,'/'];
clear CoreSettings

%instrument settings
Settings.Instruments = {'HIRDLS','MLS','SABER'}; %list of instruments to include

%geolocation - which data should we include?
Settings.HeightScale = 10:5:30;%10:1:110; %height grid to sample on.
Settings.LatRange    = [60,65];%[30,90];
Settings.LonRange    = [100,130];%[-180,180];
...
Settings.Dates.Geolocation = datenum(2007,1,21);
Settings.Dates.Sampling    = Settings.Dates.Geolocation;%datenum(2020,1,20:-1:1);
...
for iDay=1:1:numel(Settings.Dates.Geolocation)
  ...
  %params is already fine

  %weight:
  Weight = Store.Weight;
  Vars = fieldnames(Weight); 
  for iVar=1:1:numel(Vars); 
    Weight.(Vars{iVar}) = flatten(Weight.(Vars{iVar}));
    if ~strcmpi(Vars{iVar},'Format'); Weight.(Vars{iVar}) = single(Weight.(Vars{iVar})); end
  end

  %recon
  Recon = struct();
  sz = size(Store.Data.Lat);
  [Recon.x,Recon.z] = meshgrid(1:1:sz(1),1:1:sz(2));
  Recon.x = single(Recon.x(:)); Recon.z = single(Recon.z(:));

  %track
  %originalfiles, patternday, instnames
  Vars = {'Lat','Lon','Time','Alt','SourceProf','Prs','ViewAngleH','Inst','ViewAngleZ'};
  Track = struct();
  for iVar=1:1:numel(Vars); 
    Track.(Vars{iVar}) = flatten(Store.Data.(Vars{iVar}));
    if ~strcmpi(Vars{iVar},'Time'); Track.(Vars{iVar}) = single(Track.(Vars{iVar})); end
  end
  Track.PatternDay = Settings.Dates.Geolocation(iDay);
  Track.InstNames = Settings.Instruments;
  Track.OriginalFiles = Store.Data.OriginalFiles;
  disp('Data generated, saving')

  OutFile = [Settings.OutDir,'track_',Settings.Instrument,'_',num2str(Settings.Dates.Sampling(iDay)),'.mat'];
  save(OutFile,'Track','Recon','Weight','Params');


  %tidy up for next loop
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  disp(['--> ',datestr(Settings.Dates.Geolocation(iDay)),'; track files written'])
