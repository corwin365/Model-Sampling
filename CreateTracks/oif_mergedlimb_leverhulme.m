...
%instrument settings
Settings.Instruments = {'HIRDLS'}; %list of instruments to include

%geolocation - which data should we include?
Settings.HeightScale = 20:1:60; %height grid to sample on.
Settings.LatRange    = [50,70];
Settings.LonRange    = [50,70];
...
Settings.Dates.Geolocation = datenum(2020,1,21);%datenum(2020,1,28);
Settings.Dates.Sampling    = datenum(2020,1,21);%datenum(2020,1,28);

%size of geographical subset regions. To produce daily global files, set to values > [360,180,24]
Settings.Subsets = [60,30,24]; %deglon, deglat, hours
...
if strcmpi(Settings.Instruments{iInst},'HIRDLS'); Settings.Dates.Geolocation(iDay) = Settings.Dates.Geolocation(iDay) - datenum(2020,1,1) + datenum(2007,1,1); end
    ...
    Data = get_limbsounders(Settings.Dates.Geolocation(iDay),   ...
                            ...
                            'LatRange',Settings.LatRange,       ...
                            'LonRange',Settings.LonRange,       ...
                            'DateWarning',false);
  ...
  for LonBand=min(Settings.LonRange):Settings.Subsets(1):max(Settings.LonRange)
    for LatBand=min(Settings.LatRange):Settings.Subsets(2):max(Settings.LatRange)
      for TimeBand=(Settings.Dates.Geolocation(iDay):Settings.Subsets(3)/24:Settings.Dates.Geolocation(iDay)+1) 
        ...
        idx = intersect(InRange.Lon,InRange.Lat);
