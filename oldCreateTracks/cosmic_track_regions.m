...
%
%divide into smaller geographic regeions and temporal chunks, in order to 
%better fit very large models into memory
%
%
%Corwin Wright, c.wright@bath.ac.uk, 2023/06/03
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%dataset identifier
Settings.Instrument = 'COSMIC_regions';
...
% % % %   VarsOut = cat(2,VarsOut,'QC');
  ...
  RegionLat  = repmat(nanmean( Lat,2),1,size( Lat,2));
  RegionLon  = repmat(nanmean( Lon,2),1,size( Lon,2));
  RegionTime = repmat(nanmean(Time,2),1,size(Time,2));
  ...
  for LonBand=min(Settings.LonRange):Settings.LonStep:max(Settings.LonRange)
    for LatBand=min(Settings.LatRange):Settings.LatStep:max(Settings.LatRange)
      for TimeBand=iDay:Settings.TimeStep:iDay+1
        ...
        Recon  = reduce_struct(MasterRecon, idx);
