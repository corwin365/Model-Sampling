...
Settings.OutDir     = [CoreVars.MasterPath,'/tracks/gisinger/'];
Settings.PrsLevels  = CoreVars.Airs3D.HeightRange;
Settings.LatRange   = [-70,-35]+[-1,1].*30; %region of model run, +-30 deg
Settings.LonRange   = [-80,-50]+[-1,1].*30; %region of model run, +-30 deg
Settings.TimeRange  = datenum(2018,5,92:240);
...
for iDay=Settings.TimeRange(1):1:Settings.TimeRange(2)
  for iGranule=1:1:240
    ...
    %check it's in our region of interest
    GranCentre = [mean(Data.l1_lon(:)),mean(Data.l1_lat(:))];
    if GranCentre(1) < Settings.LonRange(1) ...
     | GranCentre(1) > Settings.LonRange(2) ...
     | GranCentre(2) < Settings.LatRange(1) ...
     | GranCentre(2) > Settings.LatRange(2);
      disp([datestr(iDay),', granule ',sprintf('%03d',iGranule),' not in region; skipping'])
      continue
    end
    ...
    %convert timestamps
    Data.l1_time = cjw_time_airs2matlab(Data.l1_time);
    ...
    %the z-splurging
    %we need to use a row-based approach, otherwise we'll get a jump at the end of
    %each height level
    LatScale = Data.l1_lat; NextLat = circshift(LatScale,[0,0,-1]);
    LonScale = Data.l1_lon; NextLon = circshift(LonScale,[0,0,-1]);
    ...
    Azimuth = cat(3,NaN(size(Azimuth,1),size(Azimuth,2)),Azimuth);
    Azimuth(:,:,1) = Azimuth(:,:,2);
    
    %finally, convert to a viewing angle
    ViewAngleH = wrapTo180(360-((360-Azimuth)))+45; %degrees c/w from N
    
    %and reshape to match the rest
    ViewAngleH = permute(ViewAngleH,[2,3,1]);
