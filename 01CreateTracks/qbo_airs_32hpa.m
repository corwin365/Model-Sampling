...
Settings.OutDir     = [CoreVars.MasterPath,'/tracks/AIRS_qbo/'];
Settings.PrsLevels  = 32;
Settings.LatRange   = [-1,1].*5; 
Settings.TimeRange  = CoreVars.Airs.TimeRange;
...
for iDay=Settings.TimeRange(1):1:Settings.TimeRange(2);
  ...
  %generate file name
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%  
  DayFile = [Settings.OutDir,'track_qbo_',Settings.Instrument,'_',num2str(iDay),'.mat'];
  
  if exist(DayFile);
    disp([DayFile,' already done'])
    continue
  end
