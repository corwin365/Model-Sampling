...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%load ECMWF forecast data, and put into common analysis format
...
ModelPath = '/scratch/b/b382226/ecmwf_forecasts';
...
if (TimeRange(1) - forecastTime) < 0
    Model.Error = 1;
    disp('Track occurs before the first forecast step');
    return
end
...
%check if we are after the end of the forecast (with ~2 hours padding)
tObs = min(TimeRange);
tFCMax = forecastTime+hours(max(allTimeSteps)+2);
if tObs > tFCMax; Model.Error = 3; disp('More than two hours after forecast ends!'); return; end
...
if numel(a) == 0;Model.Error = 2; return; end
